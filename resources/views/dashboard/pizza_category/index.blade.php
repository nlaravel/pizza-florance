...
@endpush
@section('content')
    <pizza-category-index
            :pizza_category='{{ $pizza_category->toJson() }}'
            :parentcategories='{!!  isset($parentCategories) ? $parentCategories : 'null' !!}'
            inline-template>
                ...
                <div class="table-responsive">
                    <table class="table data-thumb-view">
                        ...
                        <tr>
                            ...
                            <th>Price</th>
                            <th>Add Type of Ingredient/Ingredients</th>
                            <th>@lang('lang.action')</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr  v-for="property,index in pizza_category_all.data" :id="property.id">
                            <td></td>

                            <td class="product-name">@{{ property.name }}</td>
                            <td class="product-name">@{{ property.price ? property.price :'Nothing'}}</td>
                            <td class="product-name">@{{ property.category_parent ? property.category_parent.name:'Main Category' }}</td>
                    ...
                    <vs-pagination   :total="pizza_category_all.last_page"  v-model="currentx" @change="onChangePage" ></vs-pagination>
                            ...
                            <div class="data-fields px-2 mt-3">
                                <div class="row">
                                    ...
                                    <div class="col-sm-12 data-field-col">
                                        <label for="data-name"> Ingredients Name</label>
                                        <input type="text" class="form-control" id="name" v-model="name">
                                        <span v-if="form.error && form.validations.name" style="color: red">@{{ form.validations.name[0] }}</span>
                                    </div>

                                    <div class="col-sm-12 data-field-col">
                                    <label for="data-name"> Type of Ingredient</label>
                                    <div>
                                    <v-select  label="name" dir="ltr" v-model="parent_id" :options="parentcategories" :dir="$vs.rtl ? 'rtl' : 'ltr'"  />
                                    </div>
                                    <span v-if="form.error && form.validations.category_id" style="color: red">@{{ form.validations.category_id[0] }}</span>
                                    </div>
                                    <div class="col-sm-12 data-field-col"  v-if="parent_id !=null" >
                                        <label for="data-name"> Ingredients Price</label>
                                        <input type="text" class="form-control" id="price" v-model="price">
                                        <span v-if="form.error && form.validations.name" style="color: red">@{{ form.validations.price[0] }}</span>
                                    </div>
                                    <div class="col-sm-12 data-field-col"  v-if="parent_id !=null" >
                                        <label for="data-name"> Ingredients Required Or Not</label>

                                        <v-select   dir="ltr" v-model="required" :options="types" :dir="$vs.rtl ? 'rtl' : 'ltr'"  />
                                    </div>
