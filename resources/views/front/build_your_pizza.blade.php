...
@section('content')
    <section class="pizza-build-section">
        <div class="container">
            <form id="build_pizza_form" name="build_pizza_form" class="" action="" method="post">
                {{ csrf_field() }}
                ...
                <input type="hidden" value="Special Pizza" id="name" name="name">
                                ...
                                @foreach($type->childs as $child)
                                    ...
                                    <div class="col-md-6">
                                        <div class="custom-control custom-checkbox">
                                            ...
                                            <input    type="checkbox" class="custom-control-input" name="type[]"  value="{{$child->id}}" id="checkbox_type_{{$child->id}}" data-typecategory="{{$child->categoryParent->name}}" data-typeid="{{$child->id}}" data-nametype="{{$child->name}}" data-pricetype="{{$child->price}}" onClick="type_click(this)">
                                           @else
                                            <input  checked onclick="return false;" onkeydown="return false;"  type="checkbox" class="custom-control-input" name="type[]"  value="{{$child->id}}" id="checkbox_type_{{$child->id}}" data-typecategory="{{$child->categoryParent->name}}" data-typeid="{{$child->id}}" data-nametype="{{$child->name}}" data-pricetype="{{$child->price}}" onClick="type_click(this)">
                                            @endif
                ...
                <div class="col-lg-6">
                    <div class="border-box build-price">
                        <div class="d-flex justify-content-between align-items-center header">
                            <h2 class="title pb-0">1x Build Your Pizza</h2>
                            <div><span id="size_price_2">0</span> <span class="dollar d-none">$</span></div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            $('.select-pizza_size').on('change', function() {
                ...
                document.getElementById("size_price_1").textContent= $(this).find(':selected').data('price')+'$';
                document.getElementById("size_price_2").textContent= $(this).find(':selected').data('price');
                $('.dollar').addClass('d-inline');
               // $('input[type="checkbox"]').attr('disabled', false);
                let size_price=  $('#size_price_2').text();
            ...
            function type_click(e){
                ...
                // console.log(size_price_2,name,price)
                ...
                if($('#checkbox_type_'+id).is(':checked')) {
                    ...
                    let size_price=  $('#size_price_2').text().trim();
                    ...
                    $(' .box_price').each(function(){
                        ...
                        let size_price=parseFloat($('#size_price_2').text());
            ...
            $(document).ready(function() {
                ...
                $(".custom-control-input:checked").each(function () {
                   // alert("Id: " + $(this).attr("id") + " Value: " + $(this).val());
                    console.log($(this).data("typeid"),$(this).val())
                    let id= $(this).data('typeid')
                    let typecategory= $(this).data('typecategory')
                    let name= $(this).data('nametype')
                    let price= $(this).data('pricetype')
                    // console.log(size_price_2,name,price)
                ...
                $('#submit').click(function(event) {
                    var form = $('#build_pizza_form');

                    $.ajax({
                        type        : 'POST',
                        url         : '{{route("cart.addspecial")}}',
                        data        : form.serialize(),
                        dataType    : 'json',
                        encode          : true
                    }).done(function(data) {
